...
public class SwitchStatMin implements Job {
	private final Logger logger = LoggerFactory.getLogger(SwitchStatMin.class);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		List<PimSwitchSecVO> switchList = null;
		...
		try{
			switchList = repo.getSwitchSecMin((dateForm.format(now)).toString()); 		// SWITCH_STAT_10S 테이블로부터 모든 값을 select함. 
		} catch (Exception e){
			e.printStackTrace();
		}
		logger.debug("=== Start ===");
		if(switchList!=null )
		{
			for( PimSwitchSecVO sw : switchList )			
			{
				if( lastNo != sw.getNo())					// 새로운 switch에 대한 목록인 경우. 
				{
