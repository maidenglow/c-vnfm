...
public class SwitchStatHr implements Job {
	private final Logger logger = LoggerFactory.getLogger(SwitchStatHr.class);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		try{
			switchList = repo.getSwitchStatInfoHour((dateForm.format(now)).toString()); 		// SWITCH_STAT_10S 테이블로부터 모든 값을 select함. 
		} catch (Exception e){
			e.printStackTrace();
		}
		if( switchList != null )
		{
			...
			{	
				...
				insertParam.setStatType(2);				// define 필요. 
