...
.directive("nfvVnfdepEditor", ["$rootScope", function($rootScope) {
    return {
        restrict: "A",
        templateUrl: 'views/descriptor/nsd-vnfdep/vnfdep-template.html',
        replace: false,
        scope: {
            vnfdep_data: "=ngModel",
            editMode: "=ngEditMode",
        ...
        controller: ["$scope", "logger", "$q", "$timeout", "$modal", "DescriptorTreeViewAPI", 
            ...
            if ($scope.vnfdep_data != undefined) {
        		console.log('nfvVnfdepEditor vnfdep_data: ', $scope.vnfdep_data);
                ...
                if ($scope.selectedNode != undefined && $scope.selectedNode.id != undefined && $scope.selectedNode.id != '' &&
        			...
        			$q.all([DescriptorOptionsAPI.get({id:$scope.selectedNode.id.toLowerCase(), md:'get'}).$promise]).then(function(result) {
                    	...
                    	if (result[0] != undefined) {
  	              			...
  	              			$scope.vnfdep_data = model.vnfdep_list;
  	              			console.log('vnfdep_data: ', $scope.vnfdep_data);
  	              		}
            ...
            $scope.parseFormOptionsData = function () {
            	...
            	if($scope.selectedNode.id == 'NSD-VNFDEP'){
            		
            		var data = $scope.vnfdep_data;
            		
            		for (var vnfdep = 0; vnfdep < data.vnfdep.length; vnfdep++) {
                		var param = {};
                		...
                		param["vnfdep_no"].push(data.vnfdep[vnfdep].vnfdep_no);
                		param["item"].push(data.vnfdep[vnfdep].item);
                		param["checked"].push(data.vnfdep[vnfdep].checked);
                		formData += $.param(param).replaceAll("vnfdep_no%5B%5D=", "vnfdep_no=").replaceAll("item%5B%5D=", "item=").replaceAll("checked%5B%5D=", "checked=") + '&';
                	}
            ...
            $scope.$on('addNewSubitem', function (event, data) {
            	console.log('emit::addNewSubitem(): NSD-VNFDEP -', data);
            	if (data.category == 'options' && data.id == 'NSD-VNFDEP') {
            		$scope.addNewOption();
            	}
            });
            ...
            $scope.deleteSelectedItem = function () {
            	console.log('deleteSelectedItem(): ', $scope.selectedNode, $scope.vnfdep_data);
