...
angular.module("app.instances", [])
	.controller("InstancesCtrl", ["$scope", "$modal", "logger", "$q", "$timeout", "$location", "ComputeServersAPI", "ComputeServerActionsAPI", "ComputeFlavorsAPI", "ComputeImagesAPI", "ComputeLimitAPI", "NetworkingFloatingIpsAPI", "TelemetryInstancesAPI", "ComputeServersSnapShotAPI", 
	                      function($scope, $modal, logger, $q, $timeout, $location, ComputeServersAPI, ComputeServerActionsAPI, ComputeFlavorsAPI, ComputeImagesAPI, ComputeLimitAPI, NetworkingFloatingIpsAPI, TelemetryInstancesAPI, ComputeServersSnapShotAPI) {
    var STATUS_CHOICES, createRouter, createFloatingIp, createInstance, deleteFloatingIp, deleteInstance, deleteInstances, findIndex, findIndexX, findIndexY, getInstance, getInstances, getTelemetryInstance, promise, refreshInstance;
     ...
     $scope.reloadDetailData = function () {
        ...
        return ComputeServersAPI.get(instance, function(response) {
            ...
            $scope.instance = response, idx = findIndexX(instance), idx2 = findIndexY(instance), idx > -1 ? 
            		$scope.instances[idx].children.child[idx2].info = response
//            		(refreshedInstance = response, refreshedInstance.$$hashKey = $scope.instances[idx].$$hashKey, $scope.instances[idx].children.child[idx2] = refreshedInstance)
        ...
        return ComputeServersAPI.save(instance, function(response) {
            logger.logSuccess("인스턴스가 생성되었습니다."), getInstance({
                id: response.id
        ...
        for (_ref = $scope.instances, idx = _i = 0, _len = _ref.length; _len > _i; idx = ++_i)
            if (item = _ref[idx], item.id === instance.id) return idx;
        return -1
    }, findIndexX = function(instance) {
        var idx, item, _i, _len, _ref , idx2, _j, _len2, _ref2;
        for (_ref = $scope.instances, idx = _i = 0, _len = _ref.length; _len > _i; idx = _i++){
        	_ref2 = _ref[idx].children.child;
        	for (idx2 = _j = 0, _len2 = _ref2.length; _len2 > _j; idx2 = _j++){
        		if (item = _ref2[idx2], item.id === instance.id) return idx;
    		}
		}
        return -1
        ...
        for (_ref = $scope.instances, idx = _i = 0, _len = _ref.length; _len > _i; idx = _i++){
        	_ref2 = _ref[idx].children.child;
        	for (idx2 = _j = 0, _len2 = _ref2.length; _len2 > _j; idx2 = _j++){
        		if (item = _ref2[idx2], item.id === instance.id) return idx2;
    		}
		}
        return -1
    }, createRouter = function(router) {
    	router.id = $scope.instance.id;
...
//    }]
        ...
        return ComputeServerActionsAPI.action({
            ...
            response.success ? (logger.log("정상처리 되었습니다."), getInstance(instance)) : logger.logError("처리를 실패하였습니다. " + response.fault)
        })
...
}]).controller("CreateInstanceModalCtrl", ["$scope", "$modalInstance", "$q", "ComputeLimitAPI", "ComputeFlavorsAPI", "ComputeImagesAPI", "ComputeAvailabilityZonesAPI", "InstanceLimitService", "NetworkingNetworkAPI", function($scope, $modalInstance, $q, ComputeLimitAPI, ComputeFlavorsAPI, ComputeImagesAPI, ComputeAvailabilityZonesAPI, InstanceLimitService, NetworkingNetworkAPI) {
    var getLimit, original, percentage;
    $scope.selected_networks = [], $scope.networks = [], $scope.instance = {
        name: null,
        ...
        var network;
        instance.flavor && (instance.flavorRef = instance.flavor.id), instance.image && (instance.imageRef = instance.image.id), instance.key_name = "eluon-key", instance.availability_zone && (instance.availability_zone = instance.availability_zone.id), instance.networks = function() {
            var _i, _len, _ref, _results;
            for (_ref = $scope.selected_networks, _results = [], _i = 0, _len = _ref.length; _len > _i; _i++) network = _ref[_i], _results.push({
                uuid: network.id
            });
            return _results
        ...
        return $q.all([ComputeFlavorsAPI.query().$promise, ComputeImagesAPI.query().$promise, ComputeAvailabilityZonesAPI.query().$promise, NetworkingNetworkAPI.query().$promise]).then(function(result) {
            var availability_zones, flavors, images, networks;
            return flavors = result[0], $scope.flavors = flavors, flavors.length > 0 && ($scope.instance.flavor = flavors[0]), images = result[1], $scope.images = images, images.length > 0 && ($scope.instance.image = images[0]), availability_zones = result[2], $scope.availability_zones = availability_zones, availability_zones.length > 0 && ($scope.availability_zone = availability_zones[0].zoneName), networks = result[3], $scope.networks = networks, $scope.allProgress()
        })
    }, $scope.init(), $scope.removeNetworkItem = function(scope) {
        var idx, item, _i, _len, _ref;
        for ($scope.networks.push(scope), _ref = $scope.selected_networks, idx = _i = 0, _len = _ref.length; _len > _i; idx = ++_i)
            if (item = _ref[idx], item.id === scope.id) {
                $scope.selected_networks.splice(idx, 1);
                break
            }
    }, $scope.addNetworkItem = function(scope) {
        var idx, item, _i, _len, _ref;
        for ($scope.selected_networks.push(scope), _ref = $scope.networks, idx = _i = 0, _len = _ref.length; _len > _i; idx = ++_i)
            if (item = _ref[idx], item.id === scope.id) {
...
function($timeout, $route, $rootScope, $scope, $modalInstance, $modal, $q, logger, LiveMigrationInstanceListAPI, LiveMigrationRequestAPI, instance) {
    ...
    $scope.init = function() {
      $q.all([LiveMigrationInstanceListAPI.query().$promise]).then(function(result) {        	
